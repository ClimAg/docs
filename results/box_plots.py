...
for exp in ["historical", "rcp45", "rcp85"]:
    ...
    outputs[f"hiresireland_{exp}"] = xr.open_mfdataset(
        glob.glob(
            os.path.join(
                DATA_DIR, "HiResIreland", exp, "EC-EARTH", "*.nc"
            )
        ),
        chunks="auto",
        decode_coords="all"
...
for exp in ["historical", "rcp45", "rcp85"]:
    ...
    inputs[f"hiresireland_{exp}"] = xr.open_dataset(
        glob.glob(
            os.path.join(
                "data", "HiResIreland", "IE", f"*EC-EARTH*{exp}*.nc"
            )
        )[0],
        chunks="auto",
        decode_coords="all"
    ...
    inputs[f"eurocordex_{exp}"] = xr.open_dataset(
        glob.glob(
            os.path.join(
                "data", "EURO-CORDEX", "IE", f"*EC-EARTH*{exp}*.nc"
            )
        )[0],
        chunks="auto",
        decode_coords="all"
...
# %%
cplt.plot_box(
    data=inputs, lonlat=(LON_VAL, LAT_VAL), var="PP"
)

# %%
cplt.plot_box(
    data=inputs, lonlat=(LON_VAL, LAT_VAL), var="PP", fliers=True
)
...
# %%
cplt.plot_box_multi(
    data=outputs, lonlat=(LON_VAL, LAT_VAL),
...
cplt.plot_box_multi(
    data=outputs, lonlat=(LON_VAL, LAT_VAL),
    varlist=["gro", "pgro", "bm", "i_bm"], fliers=True
)
